...
- **2026-09-09**: Added `DELETE /members/:id` endpoint in `MembersController` for manual member deletion (cascading related passes, logs, and consents) and integrated "Delete Member" button in `MemberDetailPage`. Added Google Wallet pass sync check in `getMember` to automatically delete local records if the user removed their pass from Google Wallet (`hasUsers === false`).
- **2026-09-10**: Enhanced Google Wallet `createGenericClass` location payload with explicit `walletobjects#latLongPoint` kind. Added draft status tracking on template location modifications in `TemplateWorkspace`, added error handling to dashboard data fetches, and added verification scripts for Google Wallet class location inspection.
- **2026-09-10**: Added archetype field presets (`loyalty`, `membership`, `id_card`, `access_badge`) in `TemplateWorkspace` that auto-populate row configurations and update draft state. Enhanced `PassPreviewCard` with smooth transition animations, responsive typography, and archetype support.