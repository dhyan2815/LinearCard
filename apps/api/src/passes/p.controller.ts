...
export class PController {
  ...
  async redirectPass(@Param('id') id: string, @Res() res: Response) {
    try {
      if (!id) throw new HttpException('Missing pass ID', HttpStatus.BAD_REQUEST);
      ...
      if (!objectSuffixOverride) {
        throw new HttpException('Invalid pass configuration', HttpStatus.INTERNAL_SERVER_ERROR);
      }
      ...
      const passResult = await this.walletService.createGoogleWalletPass({
        ...
        logoUrl: pass.tenant.logoUrl?.startsWith('/') ? `http://localhost:3000${pass.tenant.logoUrl}` : pass.tenant.logoUrl,
        heroImageUrl: pass.tenant.heroUrl?.startsWith('/') ? `http://localhost:3000${pass.tenant.heroUrl}` : pass.tenant.heroUrl,
      ...
      if (passResult.success && passResult.googleWalletUrl) {
        ...
        throw new HttpException('Failed to generate pass URL', HttpStatus.INTERNAL_SERVER_ERROR);
      }
    } catch (err: any) {
